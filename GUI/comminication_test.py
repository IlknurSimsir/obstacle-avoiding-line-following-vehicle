...
def barkod_okuma_thread():
    ...
    try:
        barkod_ser = serial.Serial(BARKOD_SERIAL_PORT, BARKOD_BAUD_RATE, timeout=1)
        logging.info(f"BAŞARILI: GM65 barkod okuyucu {BARKOD_SERIAL_PORT} üzerinden bağlandı.")
        while True:
            veri = barkod_ser.readline().decode('utf-8').strip()
            if veri:
                barkod_verisi = veri
                logging.info(f"GM65 Barkod Okundu: {barkod_verisi}")
    except serial.SerialException as e:
        logging.error(f"HATA: GM65 barkod okuyucuya bağlanılamadı: {e}")
try:
